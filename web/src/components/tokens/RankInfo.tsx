...
const RankInfo = ({ token }: { token: OwnedToken }) => {
  ...
  const LeaderboardPosition = ({
    ...
    return (
      <Flex alignItems={"center"} gap={"10px"} justifyContent={"space-between"} w={"100%"}>
    ...
    <Popover placement={"right-start"}>
      ...
      >
        <div className={styles.leaderboardsContainer}>
          {leaderboardRecords.data && leaderboardRecords.data.batters.length > 0 && (
          ...
          {leaderboardRecords.data && leaderboardRecords.data.pitchers.length > 0 && (
            <div className={styles.leaderboardRows}>
              <Text>Pitchers</Text>
